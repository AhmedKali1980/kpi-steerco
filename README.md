...
- Gen 2 rows are enriched from Data4Sec `inventory` using `VM_<UPPER(server_uid)>` against `hostid`; the added columns are `INV_owner_app_name`, `PA_owner_id`, `INV_beneficiary`, `PA_beneficiary_id`, `PA_beneficiary_ENV`, `INV_region`, and `F_env_calculated`, placed immediately before `F_ALL_FILTERS`; empty Gen 2 enrichment values are filled with `NOT_AVAILABLE`, while non-Gen 2 rows are filled with `NOT_GEN2`
- `F_env_calculated` evaluates environment eligibility from `PA_beneficiary_ENV` for Gen 2 rows and from `server_environment` for non-Gen 2 rows, using `F_INTEXP.INCLUDE_server_environment` tokens
