...
- Gen 2 rows are enriched from Data4Sec `inventory` by looking up `VM_<UPPER(server_uid)>` in `hostid` and adding `INV_owner_app_name`, `INV_beneficiary`, and `INV_region` immediately before `F_ALL_FILTERS`
- `F_ALL_FILTERS` is appended at the end and is `Y` only if every INTERNET.EXPOSED filter evaluates to `Y`
- the XLSX also includes `DictAccount`, built from distinct `INV_owner_app_name` and `INV_beneficiary` values and enriched from `platform_accounts.tags` into `account`, `id`, and `env`
- outputs are an XLSX workbook (`RAW_INTERNET_EXPOSED`, `STATS`, and `DictAccount` sheets), optional CSV, and optional compressed JSON; the XLSX applies dark filter headers, grey filter values, light borders, and automatic column widths
