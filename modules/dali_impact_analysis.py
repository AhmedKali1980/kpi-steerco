    ...
    for col_idx, field in enumerate(fieldnames):
        ...
        if str(field).endswith(" Indicator Icon"):
            if str(field).strip() == "% servers with illumio installed Indicator Icon":
                ...
                rules.append(
                    f'<conditionalFormatting sqref="{sqref}"><cfRule type="iconSet" priority="{priority}"><iconSet iconSet="3TrafficLights1" showValue="0"><cfvo type="percent" val="100"/><cfvo type="percent" val="0" gte="false"/></iconSet></cfRule></conditionalFormatting>'
                )
            else:
